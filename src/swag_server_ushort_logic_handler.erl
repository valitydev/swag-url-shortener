-module(swag_server_ushort_logic_handler).

-export([handle_request/4]).
-export([map_error/2]).

-type operation_id()    :: swag_server_ushort:operation_id().
-type api_key()         :: swag_server_ushort:api_key().
-type auth_context()    :: swag_server_ushort:auth_context().
-type object()          :: swag_server_ushort:object().
-type request_context() :: swag_server_ushort:request_context().
-type handler_opts(T)   :: swag_server_ushort:handler_opts(T).
-type logic_handler(T)  :: swag_server_ushort:logic_handler(T).
-type response()        :: swag_server_ushort:response().
